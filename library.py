...
class Metagraph(object):
    ...
    def get_all_metapaths_from(self, source, target):
        ...
        # make all the source as one temporary node and create all the temporary edges from the source to this new node
        tmp_outvertexs = []
        for edge in self.edges:
            if edge.invertex.issubset(source):
                tmp_outvertexs.append(edge.outvertex)

        tmp_edge_list = []
        for tmp_outvertex in tmp_outvertexs:
            if Edge(source, tmp_outvertex) not in tmp_edge_list:
                tmp_edge_list.append(Edge(source, tmp_outvertex))
    ...
    def get_all_metapaths_from300(self, source, target):
        """ Retrieves all metapaths between given source and target in the metagraph.
        :param source: set
        :param target: set
        :return: list of Metapath objects
...
class MetagraphHelper:
    ...
    def generate_visualisation(self, edge_list, file_path):
        try:
            ...
            # clusters first
            for index, content in clusters.items():
